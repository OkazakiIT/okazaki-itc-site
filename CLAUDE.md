...
- **協働体制（更新・2026-06-10）**：岡﨑さんの依頼により**所有権を `OkazakiIT` へ譲渡**（方針転換。旧方針=MetamoL所有は 2026-06-09 で終了）。
  - 譲渡後の正：**`OkazakiIT/okazaki-itc-site`**・公開URL **https://okazakiit.github.io/okazaki-itc-site/**（旧 metamol URL からのリダイレクトなし）。
  - **MetamoL は譲渡承諾後、岡﨑さんによる共同編集者(write)再追加が必要**（再追加までは編集不可）。再追加後は従来どおり両者 `push` で保守可。ローカルの remote URL も `OkazakiIT/...` へ要更新。
- 残タスク：譲渡の承諾（岡﨑さん）→ MetamoL 再追加 → 新URLでのライブ検証。独自ドメイン・WebP化は「対応しない」のまま。
