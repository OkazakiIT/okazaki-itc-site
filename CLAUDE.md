...
## 現在地（随時更新）
- Sprint 0〜8 実装済み。実データ反映済み。独立ハーネスで QA **無条件 PASS** 済み（lead コントラスト 5.49:1、`:focus-visible`、未使用CSS削除、`loading="lazy"`、`width/height` 実寸化）。
- **公開済み（GitHub Pages）**：**https://metamol.github.io/okazaki-itc-site/**
  - リポジトリ Public・`main` / root から配信・`.nojekyll` 済・HTTPS 強制。**`git push`（main）で自動再デプロイ**。
- 残タスク：
  - **クライアントへ譲渡＋共同編集者追加** … 要・岡﨑さんの GitHub **ユーザー名（@ハンドル）**。メール `okajitaku1958@gmail.com` だけでは @ハンドル未特定（該当ユーザー名は404・メール検索ヒット0）。譲渡すると Pages URL が `(クライアント).github.io/...` に変わるため、OGP/canonical/robots/sitemap の絶対URLも要差し替え。
  - （任意）独自ドメイン導入（`CNAME` 設定＋絶対URL差し替えで URL を固定）。
