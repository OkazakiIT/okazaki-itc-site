...
- **協働体制（確定・2026-06-09）**：**MetamoL 所有 ＋ `OkazakiIT`（岡﨑さん）が共同編集者(write)**。岡﨑さんは承諾済・編集実績あり（GitHub上で肩書きを「代表取締役」に訂正→サイト全体に反映済み）。**所有権の譲渡はしない方針**（MetamoLの編集権を確実に維持するため）。両者とも `push` で保守可。
- 残タスク：**なし**（独自ドメイン・WebP化はいずれも「対応しない」方針で確定・2026-06-09）。企画→実装→QA→実データ反映→公開→完成度向上4スプリント→共同保守体制の確立まで完了。
